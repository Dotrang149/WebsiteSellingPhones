...
namespace WebSellingPhone.Bussiness.Service.Base
{
    public class BaseService<T> : IBaseService<T> where T : class
    {
        ...
        public virtual async Task<bool> DeleteAsync(Guid id, Guid userId)
        {
            ...
            {
                _logger.LogError("Id hoặc UserId bị rỗng!");
                throw new ArgumentNullException(nameof(id));
            }

            var entity = await _unitOfWork.GenericRepository<T>().GetByIdAsync(id, userId);
            if (entity == null)
            {
                _logger.LogError("Không tìm thấy entity!");
                return false;
            }
            ...
            if (result > 0)
                _logger.LogInformation("Xóa thành công!");
            else
                _logger.LogError("Xóa thất bại!");

            return result > 0;
        }

        public virtual async Task<T?> GetByIdAsync(Guid id, Guid userId)
        {
            if (id == Guid.Empty || userId == Guid.Empty)
            {
                _logger.LogError("Id hoặc UserId bị rỗng!");
            ...
            return await _unitOfWork.GenericRepository<T>().GetByIdAsync(id, userId);
        }
    }
}
